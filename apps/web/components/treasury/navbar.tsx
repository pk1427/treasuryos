...
export function Navbar() {
  ...
  const { mode, isOwnerVerified, isKeeperHubManaged } = useTreasurySession();
      ...
      {open ? (
        <div className="border-t border-white/10 bg-zinc-950/95 px-4 py-4 md:hidden">
          ...
          <div className="mt-3 flex flex-col gap-2">
            ...
            <StatusPill tone={mode === "manage" && isOwnerVerified ? "success" : mode === "manage" ? "warning" : "neutral"} className="justify-center">{isKeeperHubManaged ? "KeeperHub · managed" : mode === "manage" ? isOwnerVerified ? "Manage · owner verified" : "Manage · verification required" : "Analyze · read-only"}</StatusPill>
            {isConnected && address ? (
